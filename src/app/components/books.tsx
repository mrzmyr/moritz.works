      ...
      {!error &&
        ...
        data.books.map((book) => (
          <Link
            ...
            key={book.id}
            className="flex items-center justify-between group item-start hover:bg-neutral-100 dark:hover:bg-neutral-800 px-4 py-2.5 rounded-lg relative grayscale hover:grayscale-0"
          >
            ...
            <SquareArrowOutUpRight className="w-4 h-4 top-3 right-3 text-neutral-500 dark:text-neutral-400 absolute hidden group-hover:block stroke-1" />
          </Link>
        ))}
    </div>
  );
